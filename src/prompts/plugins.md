...
- Constructions PHP interdites, Faktory refuse le plugin s'il en contient une : `eval(`, `assert(`, `create_function(`, `exec(`, `shell_exec(`, `system(`, `passthru(`, `popen(`, `proc_open(`, `base64_decode(`, `unserialize(`, `$wpdb->query(`, et `file_get_contents(` sur une URL `http` (utilise `wp_remote_get()` si un appel distant est indispensable).
