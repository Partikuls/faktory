...
## Écarts constatés à l'exécution — export (2026-09-13)

Exécution réelle de `export` sur `boulangerie` (`http://localhost:8101`, conteneurs déjà debout depuis les tâches précédentes ; cumul $30,54 avant export), suivie d'une restauration manuelle de `dist/` dans un Compose neuf sur le port 8299 (`docker compose -p faktory-manualrestore`), en suivant `dist/README.md` pas à pas.

**Coût et tailles mesurées.** `export` a coûté **$0,00** (aucun appel LLM ; le coût cumulé du site est resté à $30,5426 avant et après). Message de fin de stage : `✔ export — dist/: db.sql (2.3 MB), wp-content.tar.gz (16 MB), docker-compose.prod.yml, .env.example, README.md, MANIFEST.json`. Tailles exactes sur disque : `db.sql` 2 330 804 octets (≈ 2,3 MB), `wp-content.tar.gz` 15 606 150 octets (≈ 16 MB, arrondi par `fmtSize`) — cohérentes avec `MANIFEST.json.files`.

**Port de restauration du test d'intégration.** `tests/integration/export.test.ts` provisionne le site source sur le port de base **8198** (`portBase: 8198`) et restaure `dist/` dans une seconde stack sur le port **8199** (`RESTORE_PORT = 8199`, projet Compose `faktory-itrestore`) — deux ports distincts sont donc réservés par ce test, pas un seul.

**Restauration manuelle : chaque étape du runbook a fonctionné, une étape était cassée.** `cp .env.example .env` (édité pour `SITE_PORT=8299` et des mots de passe de test), `tar -xzf wp-content.tar.gz`, `docker compose -p faktory-manualrestore -f docker-compose.prod.yml up -d --wait` (démarrage propre, les trois conteneurs terminent `Healthy`), `wp db check` (succès dès le premier essai — avec `--wait` sur `docker compose up`, WordPress a déjà écrit `wp-config.php` au moment où `wpcli` devient healthy ; la boucle « répéter jusqu'à succès » du README reste une précaution correcte mais n'a pas été nécessaire ici), `wp db import - < db.sql` (succès, < 1 s) ont tous fonctionné sans modification. **`wp yoast index --reindex` tel qu'écrit dans le README bloque en exécution non interactive** : la commande demande une confirmation (« This will clear all previously indexed objects. Are you certain you wish to proceed? ») et `docker compose exec -T` sans entrée standard ne peut pas y répondre ; `--yes` n'existe pas pour cette commande WP-CLI (`Error: Parameter errors: unknown --yes parameter`), le bon flag est `--skip-confirmation` (visible dans `wp yoast index --help`). Corrigé dans `src/export/templates.ts` (`restoreReadme`) : la ligne devient `wp yoast index --reindex --skip-confirmation` ; `export` a été relancé pour régénérer `dist/README.md` avec la commande corrigée, et `tests/unit/export-templates.test.ts` continue de passer (`toContain("wp yoast index --reindex")` reste un sous-préfixe valide). `wp rewrite flush` a réussi normalement.

**Formes des deux passes `search-replace`.** La forme brute (`wp search-replace 'https://SITE_URL_PLACEHOLDER' 'http://localhost:8299' --all-tables-with-prefix`) a trouvé **114 remplacements** sur le site réel (options, postmeta, contenu des pages, index Yoast non échappé, `wp_yoast_seo_links.url` notamment). La forme JSON-échappée (`'https:\/\/SITE_URL_PLACEHOLDER'` → `'http:\/\/localhost:8299'`) a trouvé **5 remplacements**, tous dans `wp_yoast_indexable.open_graph_image_meta` — la passe échappée n'est donc pas superflue sur ce site : sans elle, les 5 lignes `open_graph_image_meta` (JSON sérialisé par Yoast) auraient gardé l'URL locale échappée. `wp rewrite flush` a été exécuté par précaution suivant le runbook ; aucun test n'a établi qu'il était strictement indispensable (l'import de `db.sql` restaure déjà les règles de réécriture stockées), mais les permaliens (`/nos-produits/`, `/contact/`, l'article testé, `/author/admin/`) ont tous répondu 200 après la séquence complète, flush inclus.

**Contrôles de restitution.** Après la séquence complète, quatre pages ont été récupérées en HTTP sur `http://localhost:8299` : la page d'accueil (200, `<title>Maison Rivet, boulangerie artisanale à Chantenay, Nantes</title>`, classes GenerateBlocks présentes) ; `/nos-produits/` (200, `data-faktory-plugin="produits"` présent, les 5 cartes produit seedées avec leur `<h3 class="faktory-produits__title">` — Brioche tressée, Sandwich jambon-beurre, Tarte aux pommes normande, Croissant au beurre, Miche au levain — et leurs images `src="http://localhost:8299/wp-content/uploads/…"` déjà réécrites) ; `/contact/` (200, `gform_wrapper_5` présent) ; un article (200, titre correct, lien auteur `http://localhost:8299/author/admin/` réécrit et non cassé). Aucune des quatre pages ne contient plus `localhost:8101` ni `SITE_URL_PLACEHOLDER`. La stack de restauration manuelle a ensuite été détruite (`docker compose -p faktory-manualrestore -f docker-compose.prod.yml down -v`) et le répertoire temporaire supprimé, sans toucher aux conteneurs `faktory-boulangerie-*` sur le port 8101 (vérifiés toujours debout et répondant 200 après coup).

**Forme des tuples `INSERT` de `wp db export` vs `wp search-replace --export`.** `db.sql` est produit par `wp search-replace … --export` (`src/export/db.ts`), pas par `wp db export`. La forme des tuples diffère : `wp search-replace --export` écrit une espace après chaque virgule à l'intérieur d'un tuple, par exemple `('2', 'siteurl', 'https://SITE_URL_PLACEHOLDER', 'on'),`, alors que `wp db export` compacte classiquement en `('2','siteurl','https://SITE_URL_PLACEHOLDER','on'),` sans espace. Sans conséquence sur l'import (SQL valide dans les deux cas), mais à connaître si un futur outil parse `db.sql` avec une regex qui suppose l'absence d'espace.
