...
9. **Relance à 0 $.** `report.json` mémorise le hash de l'arbre de chaque page relue. Au run suivant, une page dont le hash est inchangé et dont le verdict était `ok` (aucun défaut restant) est recontrôlée et recapturée mais **pas relue** (`reused`). Supprimer `qa/report.json` force la relecture de toutes les pages.
