...
Phase 8a rerun on a fresh site, `boulangerie-8a` (2026-09-14), with the same procedure: **$15.84, 26m 47s** of stage time (spec $0.39, design $1.06, provision $0.00 in 1m 59s, plugins $1.50, pages $5.62, content $1.90, qa $5.38, export $0.00). The spec listed 11 missing facts under « Informations à compléter »; QA checked 11 URLs with one `h1` and no untranslated string on each, raised no defect on the header, menu, backgrounds, links, heading font or blog page, and left 17 `needs_human` issues — missing brief facts, placeholder images, and two minor Gravity Forms details (a submit button under 48 px, the red « (Nécessaire) » mark). Details: « Écarts et mesures » in `docs/superpowers/specs/2026-09-14-faktory-phase8a-design.md`.
