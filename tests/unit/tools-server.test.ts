...
import { wpToolHandler, createFaktoryServer, TOOL_WP, resolveSitePath, gbBuildToolHandler, gbPreviewToolHandler, TOOL_GB_BUILD, TOOL_GB_PREVIEW, FAKTORY_SERVER } from "../../src/tools/server.js";
...
describe("createFaktoryServer name", () => {
  it("defaults to the canonical name and accepts an override", () => {
    expect(createFaktoryServer(ctx).name).toBe(FAKTORY_SERVER);
    expect(createFaktoryServer(ctx, "faktory-12345678").name).toBe("faktory-12345678");
  });
});
